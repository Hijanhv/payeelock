...
| One workspace spends another's prefunded funds                | `BUYER` is immutable, so only the owning wallet can fund or settle            |
| A workspace creator keeps authority over someone else's vault | The factory holds no funds and no vault roles; it only records the address    |
| The factory is impersonated by a lookalike contract           | The interface reads `BUYER` from the created vault before showing it as yours |

## Workspace creation safety

`createWorkspace` is permissionless, which is intentional: isolation comes from the vault, not from who deployed it. Each deployment fixes `BUYER` to the caller, so a workspace can only ever spend the balance its own buyer prefunded. A hostile caller can create as many workspaces as they pay gas for; that gives them no authority over anyone else's vault.

The interface treats the created address as authoritative only after decoding the factory's `WorkspaceCreated` event and reading `BUYER` back from the new vault. A user who pastes a vault address manually sees the same `BUYER` value read directly from the chain, so a phishing vault that reports someone else's buyer is visible before any funds move.

The factory is not an upgradeable proxy and holds no privileged role. Replacing it does not affect vaults it already created.
...
- Workspaces created in the browser default guardian, executor, and recovery signer to the buyer's own wallet. Reassign them to independent parties before using real value.
- The public test token has an open mint function and must never be treated as value.
- The factory does not verify that a workspace's chosen asset is a legitimate token.
